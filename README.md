...
- 周期发送：按官方频率调度（0x0001/0x0101/0x0105/0x0203/0x020B~0x020E 1Hz、
  ...
  比赛阶段剩余时间自动倒计时。
- 比赛流程：一键运行“未开始 → 准备 → 十五秒自检 → 五秒倒计时 → 比赛 → 结算 →
  结束”全流程；比赛中血量随时间/伤害下降、热量随射击累积并按冷却衰减、
  底盘功率/缓冲能量波动、发弹量消耗、金币累积、增益阶段性开启，
  并自动触发 0x0207 射击、0x0206 伤害、0x0104 判罚、0x0002 比赛结果事件。
- 自定义客户端：模拟官方自定义客户端协议（MQTT + Protobuf，服务器默认
  192.168.12.1:3333，客户端 ID 为机器人 ID）。内置全部 36 条官方消息定义，
  可发布键盘鼠标/自定义数据/通用指令/飞镖/哨兵等指令，并自动订阅接收
  GameStatus/RobotStaticStatus/RobotDynamicStatus 等服务器消息，实时字段解码。
- 接收/解析：实时显示上下行帧，字段级解码；0x0301 的 UI 图形按官方
  1920×1080 坐标系绘制到画布（直线/矩形/圆/椭圆/圆弧/浮点/整型/字符），
  支持删除图层/清空操作；0x0305 机器人位置与 0x0307 路径绘制到 28m×15m
  小地图（红=对方、蓝=己方、黄线=路径），0x0308 自定义消息按 UTF-16LE
